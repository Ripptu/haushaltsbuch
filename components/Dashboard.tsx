  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
           <h1 className="text-2xl md:text-3xl font-light text-white">Hallo, Designer.</h1>
        </div>
        
        <div className="bg-zinc-900/80 backdrop-blur p-1 rounded-xl border border-white/5 inline-flex self-start md:self-auto">
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <motion.div 
          ...
          className="md:col-span-2 relative overflow-hidden rounded-3xl bg-vamela-surface border border-white/5 p-6 md:p-8 flex flex-col justify-between h-[300px] md:h-[340px]"
        >
          <div className="z-10">
            <h3 className="text-zinc-500 text-xs md:text-sm font-medium uppercase tracking-widest mb-1">
              {timeRange === 'all' ? 'Aktueller Kontostand' : 'Saldo (Monat)'}
            </h3>
            ...
            <div className="flex items-center gap-2 mt-4">
              <div className="px-3 py-1 rounded-full text-xs font-medium bg-zinc-800 text-zinc-400 border border-white/5">
                 {timeRange === 'all' ? 'Netto Vermögen' : 'Netto Ergebnis'}
              </div>
            </div>
          </div>
          
          <div className="absolute bottom-0 left-0 right-0 h-[200px] opacity-80 pointer-events-none">
        ...
        {/* Quick Stats Column */}
        <div className="space-y-4 md:space-y-6 flex flex-col md:h-[340px]">
            {/* Income */}
            <motion.div 
              layout
            ...
            >
               ...
               <span className="text-2xl md:text-3xl font-medium text-white mt-1 tracking-tight">
               ...
               >
                 <div className="flex justify-between items-center mb-3">
                   ...
                   <span className={`text-xs font-mono px-2 py-0.5 rounded border ${m.net >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                     {m.net >= 0 ? '+' : ''}{m.net.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                   </span>
                 </div>
                 <div className="space-y-1">
                   <div className="flex justify-between text-xs text-zinc-500">
                     <span>Einnahmen</span>
                     <span className="text-emerald-500/80">+{m.income.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</span>
                   </div>
                   <div className="flex justify-between text-xs text-zinc-500">
                     <span>Ausgaben</span>
                     <span className="text-zinc-400">-{m.expense.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</span>
                   </div>
                 </div>
               </motion.div>
             ))}
              {monthlyData.length === 0 && (
                 <div className="text-zinc-600 text-xs">Keine Daten verfügbar.</div>
              )}
          </div>
        </div>
      </div>
